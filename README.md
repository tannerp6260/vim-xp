...
An early client-side Vim apprenticeship built on CodeMirror 6 + `@replit/codemirror-vim`. `#/practice` provides focused sessions across two units: precise text-object edits and line-local targeting with `f`, `t`, `;`, `,`, change, and delete. `#/curriculum` shows both units without locking advanced learners out; the diagnostic laboratory remains at `#/lab`.

Progress stays in versioned browser-local storage. Schema 3 migrates compatible schema-2/curriculum-2.0.0 progress without losing learner evidence or valid Unit 1 sessions. There are no accounts, backend, telemetry, runtime AI, timers, leaderboards, or broad Vim compatibility claims.
...
6. Open the URL Vite prints (normally `http://localhost:5173/#/practice`). Curriculum is at `http://localhost:5173/#/curriculum`; the diagnostic laboratory remains at `http://localhost:5173/#/lab`.
...
See [local development](docs/local-development.md), [architecture](docs/architecture.md), [content authoring](docs/content-authoring.md), [learning and planning](docs/learning-and-planning.md), [persistence](docs/persistence.md), [engine evidence](docs/engine-evaluation.md), and [deployment](docs/deployment.md).
