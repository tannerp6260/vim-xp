...
const score = (exercise: Exercise, learner: LearnerState, now: number, roll: number) => {
  const state = learner.concepts[exercise.primaryConcepts[0]]
  if (!state) return 70 - exercise.difficulty.level * 3 + roll
  const due = state.dueAt <= now ? 45 : 0
  const weak = (1 - state.strength) * 50
  const slowBall = state.strength >= .8 ? (roll > .96 ? 80 : -25) : 0
  return due + weak + slowBall - (state.recentExerciseIds.includes(exercise.id) ? 35 : 0) + roll
}

export function planSession(curriculum: Curriculum, learner: LearnerState, clock: Clock, seed: number, firstSession: boolean): SessionPlan {
  const now = clock.now()
  if (firstSession) return { id: `session-${now}-${seed}`, exerciseIds: [...firstSessionExerciseIds], prescribed: true, createdAt: now, seed }
  const rand = random(seed)
  const ranked = curriculum.exercises.map((exercise) => ({ exercise, score: score(exercise, learner, now, rand()) })).sort((a, b) => b.score - a.score || a.exercise.id.localeCompare(b.exercise.id))
