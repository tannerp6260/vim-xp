...
describe('session planner', () => {
  ...
  it.each(['unit.move-and-repeat', 'unit.search-and-act'] as const)('uses the seven-exercise prescribed introduction for %s', (unitId) => { const plan = planSession(curriculum, emptyLearnerState(), clock, 1, false, [], unitId); expect(plan).toMatchObject({ unitId, prescribed: true }); expect(plan.exerciseIds).toEqual(curriculum.units.find((unit) => unit.id === unitId)!.prescribedExerciseIds) })
  ...
  it('draws mixed adaptive review across all four units', () => { const plan = planSession(curriculum, emptyLearnerState(), clock, 17, false); const units = new Set(plan.exerciseIds.map((id) => curriculum.units.find((unit) => unit.exerciseIds.includes(id))!.id)); expect(units).toEqual(new Set(curriculum.units.map((unit) => unit.id))) })
