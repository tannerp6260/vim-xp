...
export function PracticeScreen() {
  ...
  const cancelDemo = useCallback(() => { demoAbortRef.current?.abort(); demoAbortRef.current = null; setDemonstrating(false); setDemoPending(false) }, [])
  const reset = useCallback(() => { cancelDemo(); setSnapshot(initialSnapshot(exercise)); setFeedback(null); setComplete(false); setGeneration((value) => value + 1) }, [cancelDemo, exercise])
  ...
  useEffect(() => { if (!demoPending) return; const controller = new AbortController(); demoAbortRef.current = controller; const timer = window.setTimeout(async () => { setDemonstrating(true); setDemonstrated(true); await adapterRef.current?.replay(exercise.referenceSolutions[0].tokens, 90, controller.signal); if (!controller.signal.aborted) { setDemoPending(false); setDemonstrating(false); setFeedback({ kind: 'success', message: 'Demonstration complete. Reset, then reproduce the edit yourself.' }) } }, 0); return () => { window.clearTimeout(timer); controller.abort() } }, [demoPending, exercise, generation])

  const advance = (skipped: boolean) => {
    if (demonstrating || demoPending) return
    ...
    cancelDemo(); setHintCount(0); setFeedback(null); setComplete(false); setIncorrectChecks(0); setDemonstrated(false); setSnapshot(initialSnapshot(finished ? exercise : getExercise(session.exerciseIds[nextIndex]))); setGeneration((value) => value + 1)
  }
  const demonstrate = () => { reset(); setDemoPending(true) }
  const clearTransient = (nextExercise: Exercise) => { cancelDemo(); setHintCount(0); setFeedback(null); setComplete(false); setIncorrectChecks(0); setDemonstrated(false); setSnapshot(initialSnapshot(nextExercise)); setGeneration((value) => value + 1) }
  ...
  useEffect(() => { const listener = (event: KeyboardEvent) => { if (event.ctrlKey && event.key === 'Enter') { event.preventDefault(); if (demonstrating || demoPending) return; if (complete) advance(false); else check() } }; window.addEventListener('keydown', listener, true); return () => window.removeEventListener('keydown', listener, true) })
  ...
  const busy = demonstrating || demoPending
