...
import { gateConfirmed } from '../learning/placement'
...
export function CurriculumScreen() {
  ...
  const recommended = recommendNext(curriculum, progress.learner, progress.placement)
  ...
  return <main className="curriculum-screen"><ProductNav /><header><p className="lesson-label">Small, deliberate curriculum</p><h1>Choose what to practice</h1><p>Units are recommended in order, but never locked. Progress reflects learner evidence, not a completion percentage.</p><p><a className="button-link" href="#/placement">{progress.placement ? (progress.placement.status === 'active' ? 'Resume placement' : 'Retake placement') : 'Find my starting point'}</a></p><p>{recommended.reason}</p></header>
    ...
    <section className="unit-list" aria-label="Curriculum units">{[...curriculum.units].sort((a, b) => a.order - b.order).map((unit) => <article className="unit-card" key={unit.id} data-unit-id={unit.id}>
      <div className="unit-meta"><span>Unit {unit.order}</span><span>{unitStatus(unit, progress, now)}</span>{recommended.kind === 'unit' && recommended.unitId === unit.id && <strong>Recommended</strong>}</div>
      ...
      {curriculum.placementGates.filter((gate) => gate.unitId === unit.id && gate.requiredToSkipIntroduction).every((gate) => gateConfirmed(gate, progress.placement, progress.learner)) && <p className="placement-skipped">Introduction skipped by placement</p>}{unit.recommendedPrerequisiteUnitIds.length > 0 && <p className="prerequisite">Recommended first: {unit.recommendedPrerequisiteUnitIds.map((id) => getUnit(id)!.title).join(', ')}. You can start here now if this material is familiar.</p>}
