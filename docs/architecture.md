...
Effects are derived from actual before/after snapshots, including inserted, removed, or replaced text and mode changes. The adapter marks the resulting range or nearest deletion boundary temporarily; the same effect is always status text. Direct editing and practice navigation remain locked while the player is open. Next, Previous, and Restart share one operation controller: starting an operation cancels its predecessor, reconstruction checks currency after editor recreation and every replay, and adapter typing delays settle immediately on abort. Timers, animation-frame waits, replay, reconstruction, exit, local reset, exercise changes, navigation, and unmounting therefore share the same cancellation boundary. The one-active-editor constraint is unchanged.
