...
Effects are derived from actual before/after snapshots, including inserted, removed, or replaced text and mode changes. The adapter marks the resulting range or nearest deletion boundary temporarily; the same effect is always status text. Direct editing and practice navigation remain locked while the player is open, and replay is abort-safe across pause, reconstruction, exit, local reset, exercise changes, navigation, and unmounting. The one-active-editor constraint is unchanged.
