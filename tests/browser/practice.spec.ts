...
test('supports incorrect recovery, intended completion, progress, and isolated transition', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Continue' }).click(); await expect(page.getByText('2 of 7')).toBeVisible(); await expect(page.getByRole('heading', { name: 'Switch the build type' })).toBeVisible()
...
test('skip resumes after reload and session reaches a clear completion', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Seven precise edits, done.' })).toBeVisible(); await expect(page.getByText('Inside quotes')).toBeVisible()
...
test('every catalog reference replays through the real Vim adapter and satisfies its outcome', async ({ page }) => {
  ...
  for (const [id, keys, inserted] of references) {
    await page.evaluate(({ exerciseId }) => localStorage.setItem('vim-xp-progress', JSON.stringify({ schemaVersion: 1, curriculumVersion: '2.0.0', learner: { concepts: {}, attempts: [] }, recentVariants: [], session: { id: 'reference-test', exerciseIds: [exerciseId], prescribed: false, createdAt: 1, seed: 1, index: 0, completed: false } })), { exerciseId: id })
    await page.reload(); await expect(editor(page)).toBeVisible()
    for (const key of keys) await page.keyboard.press(key)
    if (inserted) await page.keyboard.type(inserted)
    await page.keyboard.press('Escape'); await page.getByRole('button', { name: 'Check', exact: false }).click()
